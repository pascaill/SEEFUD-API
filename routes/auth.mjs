...
export const register = async (req, res) => {
  ...
  if (!name || !email || !password ||!role) {
    return res.status(400).json({ error: 'Please provide name, email, and password, role' });
  }
  if (role !== 'user' && role !== 'pemilik_umkm') {
    return res.status(400).json({ error: 'Role must be either "user" or "pemilik_umkm"' });
  }
  try {
...
export const deleteAccount = async (req, res) => {
  ...
  if (email !== userEmail) {
    return res.status(403).json({ status: 'failed', message: 'Email does not match registered email' });
  }
  ...
  try {
    ...
    return res.status(500).json({ status: 'failed', message: 'Could not delete account', error: error.message });
  }
};
...
export const updateProfile = async (req, res) => {
  ...
  if (!name && !email) {
    return res.status(400).json({ error: 'Please provide information to update' });
  }
